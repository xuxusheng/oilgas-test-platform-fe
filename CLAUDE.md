...
- **多列表格问题：** 当 ProTable 列数较多（≥8列）时，在小屏幕宽度下会出现列文字被压缩、换行或挤压的问题
- **推荐解决方案：** 使用内部横向滚动而非自动换行或隐藏列
  - 在 ProTable 上添加 `scroll={{ x: 'max-content' }}` 属性
  - 为所有列设置合理的固定宽度（如：ID: 60px, 文本列: 120-150px, 时间列: 180px）
  - 关键列使用固定定位：`fixed: 'left'`（左侧固定）和 `fixed: 'right'`（右侧固定）
  - **操作列建议固定在右侧，宽度根据按钮数量调整：**
    - 2个按钮（编辑+删除）：`width: 84-100, fixed: 'right'`
    - 3个按钮（编辑+启用/禁用+删除）：`width: 120, fixed: 'right'`
- **优势：** 保持数据完整性、操作便捷、用户体验好、不影响页面整体布局
- **示例：**
  ```tsx
  <ProTable
    ...
    columns={[
      ...
      { title: '设备编号', dataIndex: 'deviceNo', width: 120, ellipsis: true },
      // ... 其他列
...
- **2个操作按钮**（如：编辑 + 删除）：`width: 84-100px`
  - 示例：检测设备管理页面
- **3个操作按钮**（如：编辑 + 启用/禁用 + 删除）：`width: 120px`
  - 示例：油样管理、测试工位管理页面
- **原则：** 每个链接按钮约 35-40px（含间距），根据实际按钮数量和文字长度调整
- **固定定位：** 操作列必须设置 `fixed: 'right'`，确保在横向滚动时始终可见
